...
def batch_get_pelvis_orient_svd(rel_pose_skeleton, rel_rest_pose, parents, children, dtype):
    ...
    # rel_pose_skeleton 每个节点减去其父节点的 kp (非 T pose)
    # rel_rest_pose 每个节点减去其父节点的 kp (T pose)
    rest_mat = []
    target_mat = []
    ...
    rest_mat = torch.cat(rest_mat, dim=2) #  (n, 3, m)
    target_mat = torch.cat(target_mat, dim=2) # (n, 3, m)
    S = rest_mat.bmm(target_mat.transpose(1, 2)) # (n, 3, 3)
